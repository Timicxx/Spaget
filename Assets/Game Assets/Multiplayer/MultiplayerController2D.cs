...
public class MultiplayerController2D : RayCastController {
    ...
    public override void Start() {
        base.Start();
        player = transform.GetComponent<MultiPlayer>();
    }
    ...
    void VerticalCollision(ref Vector3 velocity) {
        ...
        for (int i = 0; i < verticalRays; i++) {
            ...
            if (hit) {
                player.HitHandler(hit, trap);

                if(hit.transform.tag == "nextLevel" || hit.transform.tag == "Trap") {
                    return;
                }
    ...
    void DescendSlope(ref Vector3 velocity) {
        ...
        RaycastHit2D maxSlopeHitLeft = Physics2D.Raycast(raycastPoints.bottomLeft, Vector2.down, Mathf.Abs(velocity.y) + skinWidth, collisionMask);
        RaycastHit2D maxSlopeHitRight = Physics2D.Raycast(raycastPoints.bottomRight, Vector2.down, Mathf.Abs(velocity.y) + skinWidth, collisionMask);
